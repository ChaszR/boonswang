...
if ( !class_exists( 'Attachments' ) ) :
    ...
    class Attachments {
        ...
        function meta_box_markup( $post, $metabox )
        {
            ...
            <script type="text/javascript">
                jQuery(document).ready(function($){
					...
					$element.on( 'click', 'a.delete-attachment', function( event ) {
					...
					$element.on( 'click', '.edit', function( event ) {
                        ...
                        targetAttachment = $(this).parents('.attachments-attachment');
                        // targetAttachment.find('.attachment-meta').fadeOut(125);
                        targetAttachment.addClass('flip');
                    } );
                    
                    
                    $element.on( 'click', '.done', function( event ) {
                        var targetAttachment;
                        event.preventDefault();
                        targetAttachment = $(this).parents('.attachments-attachment');
                        // targetAttachment.find('.attachment-meta').fadeOut(125);
                        targetAttachment.removeClass('flip');
                    } );
		...
		{
			?>
                    <div class="attachments-attachment attachments-attachment-<?php echo $instance; ?> panel">
                    	...
                    	<div class="front">
                    		...
                    		<div class="attachment-edit">
								...
								<a class="delete-attachment" href="#">Delete</a>
                    		</div>
                    	</div><!-- end front -->
        ...
        function admin_notice()
        {
            ...
            if( $this->has_outstanding_legacy_data() && ( isset( $_GET['page'] ) && $_GET['page'] !== 'attachments' || !isset( $_GET['page'] ) ) ) : ?>
                <div class="message updated" id="message">
                    <p>Attachments has detected legacy Attachments data. A lot has changed since Attachments 1.x.</p>
                </div>
            <?php endif;
        }
        ...
        {
            $pointer_content  = "<h3>Attachments 3.0 brings big changes!</h3>";
            $pointer_content .= "<p>'It is very important that you take a few minutes to see what has been updated. The changes will affect your themes/plugins</p>";
            ?>
