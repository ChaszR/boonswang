...
$google_fonts = array(	array( 'name' => "Cantarell", 'variant' => ':r,b,i,bi'),
						...
						array( 'name' => "Source Sans Pro", 'variant' => ':200,300')
);
...
******************************************************************/
if (!function_exists( "mc_google_webfonts")) {
	function mc_google_webfonts() {
		global $google_fonts;
		$fonts = '';
		$output = '';
...
add_action( 'wp_head', 'mc_google_webfonts' );




/******************************************************************
				 Cufon FONTS FUNCTION
******************************************************************/
//Cufon Fonts
$cufon_fonts_path = get_stylesheet_directory(). '/fonts/cufon/';
$cufon_fonts = array();

if ( is_dir($cufon_fonts_path) ) {
    if ($cufon_fonts_dir = opendir($cufon_fonts_path) ) { 
        while ( ($cufon_fonts_file = readdir($cufon_fonts_dir)) !== false ) {
            if(stristr($cufon_fonts_file, ".js") !== false) {
                $cufon_fonts[] = $cufon_fonts_file;
            }
        }    
    }
}

if (!function_exists( "mc_cufon_fonts")) {
	function mc_cufon_fonts() {
		global $cufon_fonts;
		global $data;
		$enabled = $data["enable_font_replacement"];
		echo '<!-- '.$enabled.' -->';

					if(strstr($_SERVER['REQUEST_URI'], 'wp-admin/themes.php?page=optionsframework') || strstr($_SERVER['REQUEST_URI'], 'wp-admin/themes.php?page=optionsframework')) {
					if ( !empty($cufon_fonts) ) {
						foreach($cufon_fonts as $font){
							echo '<script type="text/javascript" src="'.get_template_directory_uri().'/fonts/cufon/'.$font.'"/></script>';
						}
					}	
				}			
	}
}
add_action( 'admin_head', 'mc_cufon_fonts' );


/******************************************************************
				 FONTFACE FONTS FUNCTION
******************************************************************/
//FontFace Fonts
$fontface_fonts_path = get_stylesheet_directory(). '/fonts/fontface/';
$fontface_fonts = array();

if ( is_dir($fontface_fonts_path) ) {
    if ($fontface_fonts_dir = opendir($fontface_fonts_path) ) { 
        while ( ($fontface_fonts_file = readdir($fontface_fonts_dir)) !== false ) {
            if(stristr($fontface_fonts_file, ".ttf") !== false) {
                $fontface_fonts[] = $fontface_fonts_file;
            }
        }
    }
}
...
if (!function_exists( "mc_fontface_fonts")) {
	function mc_fontface_fonts() {
		...
		global $data;
						...
						if ( !empty($fontface_fonts) ) {
							...
							foreach($fontface_fonts as $font){
								$font = str_replace(".ttf", "", $font);
								$themeuri = get_template_directory_uri();
								echo "@font-face {
								    font-family: '$font';
								    src: url('$themeuri/fonts/fontface/$font.eot');
								    src: url('$themeuri/fonts/fontface/$font.eot?#iefix') format('embedded-opentype'),
								         url('$themeuri/fonts/fontface/$font.woff') format('woff'),
								         url('$themeuri/fonts/fontface/$font.ttf') format('truetype'),
								         url('$themeuri/fonts/fontface/$font.svg#$font') format('svg');
								    font-weight: normal;
								    font-style: normal;
...
add_action( 'admin_head', 'mc_fontface_fonts' );
?>