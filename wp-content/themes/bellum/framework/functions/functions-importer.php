...
require get_template_directory() .'/framework/functions/parsers.php';
 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class mcstudios_WP_Import extends WP_Importer {
	...
	function mcstudios_WP_Import() { /* nothing */ }
	...
	function dispatch() {
			...
			$step = empty( $_GET['step'] ) ? 0 : (int) $_GET['step'];		
				//check_admin_referer( 'import-wordpress' );				
				$file = get_stylesheet_directory() . '/includes/demo-data.xml';
				if (file_exists($file) ) {					
	...
	function import_end() {
		...
		echo '<p id="successmessagecomplete">Importcomplete</p>';
	...
	function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>Sorry, there has been an error.</strong><br />';
			echo 'The export file could not be found. It is likely that this was caused by a permissions problem.';	
		...
		if ( $this->version > $this->max_wxr_version ) {
			...
			echo 'This WXR file may not be supported by this version of the importer. Please consider updating.';
	...
	function get_authors_from_import( $import_data ) {
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					echo 'Failed to import author. Their posts will be attributed to the current user.';
...
<?php if ( $this->allow_create_users() ) : ?>
	<p><?php 
		echo 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.';
...
<?php
	...
	function author_select( $n, $author ) {
		...
		wp_dropdown_users( array( 'name' => "user_map[$n]", 'multi' => true, 'show_option_all' => __( '', 'wordpress-importer' ) ) );
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					echo 'Failed to create new user for %s. Their posts will be attributed to the current user.';
					//printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'wordpress-importer' ), esc_html($this->authors[$old_login]['author_display_name']) );
					if ( defined('MC_IMPORT_DEBUG') && MC_IMPORT_DEBUG )
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				echo 'Failed to import category';
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				echo 'failed to import post tags';
	...
	function process_posts() {
		foreach ( $this->posts as $post ) {
			if ( ! post_type_exists( $post['post_type'] ) ) {
				echo 'Failed to import &#8220;%s&#8221;: Invalid post type %s';
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				echo 'Some post, pages, categories already exist';
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							echo 'Failed to import taxonomy';
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_id ) {
			echo 'Menu item skipped due to invalid menu slug';
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( $headers['response'] != '200' ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', 'Remote server returned error response');
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', 'Remote file is too large');
		}
	 ...
	 */
	function parse( $file ) {
		$parser = new MCStudios_WXR_Parser();
	...
	function header() {
		...
		if ( isset( $updates[$basename] ) ) {
			...
			echo '<div class="error"><p><strong>';
			echo 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files';
...
} // class_exists( 'mcstudios_WP_Importer' )

function mcstudios_wordpress_importer_init() {
	 ...
	 */
	$GLOBALS['wp_import'] = new mcstudios_WP_Import();
	register_importer( 'mcstudiosinstalldemocontent', 'MC Importer', 'MC Studios Custom Importer.', array( $GLOBALS['wp_import'], 'dispatch' ) );
}
add_action( 'admin_init', 'mcstudios_wordpress_importer_init' );
