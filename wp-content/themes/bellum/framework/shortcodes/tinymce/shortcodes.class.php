...
class mc_shortcodes
{
	...
	function formate_shortcode()
	{
		...
		if( isset( $mc_shortcodes[$this->popup]['child_shortcode'] ) )
			$this->has_child = true;
		
		if( isset( $mc_shortcodes ) && is_array( $mc_shortcodes ) )
		{
			// get shortcode config stuff
			$this->params = $mc_shortcodes[$this->popup]['params'];
			$this->shortcode = $mc_shortcodes[$this->popup]['shortcode'];
			$this->popup_title = $mc_shortcodes[$this->popup]['popup_title'];
			...
			if( isset( $mc_shortcodes[$this->popup]['no_preview'] ) && $mc_shortcodes[$this->popup]['no_preview'] )
			{
			...
			{
				// set child shortcode
				$this->cparams = $mc_shortcodes[$this->popup]['child_shortcode']['params'];
				$this->cshortcode = $mc_shortcodes[$this->popup]['child_shortcode']['shortcode'];
			
				// popup parent form row start
				$prow_start  = '<tbody>' . "\n";
				$prow_start .= '<tr class="form-row has-child">' . "\n";
				$prow_start .= '<td><a href="#" id="form-child-add" class="button-secondary">' . $mc_shortcodes[$this->popup]['child_shortcode']['clone_button'] . '</a>' . "\n";
